...
data {
  ...
  vector[J] max_temp2_mean;
  vector[J] density2_mean;
} 
...
transformed data {
    ...
    for (j in 1:J) {
        ...
        X_means[j,13] = max_temp2_mean[j];  
        X_means[j,14] = density2_mean[j];  
   }
  ...
  // marginal effects of city-level variables. See email 8/16/24 from Andy Lin
   ...
   for (i in 1:L+1) {
      ...
      if(i == 8 || i == 9) { // binary
	    ...
	    if (i == 12 || i == 13 || i == 14) { // combined effect of linear and squared term
	       if (i == 12 ) 
                   k = 5; // min temp linear term
	       else if (i == 13 ) 
                   k = 10; // max temp
               else if (i == 14 ) 
                   k = 3; // density linear term
               else
                   k = 0; // should throw an error
               p2_bike_u = inv_logit(X[n,] * beta_bike[, country_num[n]] + epsilon*beta_bike[k, country_num[n]] - X[n,i]*beta_bike[i, country_num[n]] + (X[n,k]+epsilon)^2*beta_bike[i, country_num[n]] );
               p2_bike_l = inv_logit(X[n,] * beta_bike[, country_num[n]] - epsilon*beta_bike[k, country_num[n]] - X[n,i]*beta_bike[i, country_num[n]] + (X[n,k]-epsilon)^2*beta_bike[i, country_num[n]] );
               p2_walk_u = inv_logit(X[n,] * beta_walk[, country_num[n]] + epsilon*beta_walk[k, country_num[n]] - X[n,i]*beta_walk[i, country_num[n]] + (X[n,k]+epsilon)^2*beta_walk[i, country_num[n]] );
               p2_walk_l = inv_logit(X[n,] * beta_walk[, country_num[n]] - epsilon*beta_walk[k, country_num[n]] - X[n,i]*beta_walk[i, country_num[n]] + (X[n,k]-epsilon)^2*beta_walk[i, country_num[n]] );
            }
	    else {
